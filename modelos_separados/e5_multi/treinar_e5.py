...
# Modelo Multilingual-E5-Large - MICROSOFT
E5_MODEL = 'intfloat/multilingual-e5-large'
BATCH_SIZE = 32  # 560M params, batch médio para 12GB
EMBEDDING_DIM = 1024  # Dimensão fixa
MAX_SEQ_LENGTH = 512  # Limite padrão (trunca automaticamente)

# Diretórios
CHECKPOINT_DIR = 'checkpoints_e5'
RESULTS_DIR = 'results_e5'
EMBEDDINGS_DIR = 'embeddings_e5'
...
def add_e5_prefix(texts, is_query=True):
    """
    E5 requer prefixos específicos:
    - Queries: 'query: '
    - Passages/Documents: 'passage: '

    Para classificação, tratamos como queries
    """
    prefix = "query: " if is_query else "passage: "
    return [prefix + text for text in texts]

def get_e5_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings E5-Large - OTIMIZADO PARA GPU
    """
    ...
    # Adicionar prefixo E5
    texts_with_prefix = add_e5_prefix(texts, is_query=True)
    log_print(f"    ✓ Prefixo E5 adicionado ('query: ')", log_file)
    ...
    embeddings = model.encode(
        ...
        normalize_embeddings=True,  # E5 se beneficia de normalização
        device=device,
    )
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings E5-Large (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {E5_MODEL}", log_file)

            X = get_e5_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_e5.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_e5_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM MULTILINGUAL-E5-LARGE", log_file)
    log_print(f"Microsoft | 560M params | Dim: 1024 | 100 línguas", log_file)
    ...
    log_print(f"\n📥 Carregando E5-Large: {E5_MODEL}", log_file)
    log_print(f"⚠️  Modelo 560M params (XLM-RoBERTa-large base)", log_file)
    ...
    log_print(f"✓ Batch size: {BATCH_SIZE}", log_file)
    log_print(f"✓ Prefixo: 'query: ' (E5 requirement)", log_file)
    ...
    start = time.time()
    _ = model.encode(
        add_e5_prefix(test_texts, is_query=True),
        batch_size=BATCH_SIZE,
    ...
    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_e5.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_e5.pkl')
