...
# Modelo Jina-Embeddings-v3 - VERSÁTIL E POTENTE
JINA_MODEL = 'jinaai/jina-embeddings-v3'
BATCH_SIZE = 24 # Modelo médio (570M), batch bom para 12GB
EMBEDDING_DIM = 1024  # Dimensão padrão (suporta Matryoshka)
MAX_SEQ_LENGTH = 4096  # Suporta até 8192 tokens!
TASK = 'text-matching'  # Task-specific LoRA adapter

# Diretórios
CHECKPOINT_DIR = 'checkpoints_jina'
RESULTS_DIR = 'results_jina'
EMBEDDINGS_DIR = 'embeddings_jina'
...
def get_jina_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings Jina-v3 - COM TASK-SPECIFIC LORA
    """
    ...
    log_print(f"    Task: {TASK}", log_file)
    ...
    # Jina-v3 suporta parâmetros especiais:
    # - task: usa LoRA adapter específico
    # - dimensions: Matryoshka (pode truncar)
    # - late_chunking: para textos longos
    ...
    # Adicionar task se modelo suportar
    try:
        embeddings = model.encode(
            texts,
            task=TASK,  # Task-specific LoRA
            **encode_kwargs
        )
        log_print(f"    ✓ Usando task-specific adapter: {TASK}", log_file)
    except:
        # Fallback sem task parameter
        embeddings = model.encode(texts, **encode_kwargs)
        log_print(f"    ✓ Sem task-specific adapter (fallback)", log_file)
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings Jina-v3 (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {JINA_MODEL}", log_file)

            X = get_jina_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_jina.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_jina_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM JINA-EMBEDDINGS-V3", log_file)
    log_print(f"Modelo: 570M params | Dim: 1024 | Task-Specific LoRA", log_file)
    ...
    log_print(f"\n📥 Carregando Jina-v3: {JINA_MODEL}", log_file)
    log_print(f"⚠️  Modelo 570M params com task-specific LoRA", log_file)

    model = SentenceTransformer(JINA_MODEL, device=device, trust_remote_code=True)
    ...
    test_texts = ["teste de velocidade"] * 200
    start = time.time()
    ...
    if velocidade < 15:
        log_print(f"   ⚠️  Velocidade baixa - pode estar em CPU", log_file)
    else:
    ...
    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_jina.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_jina.pkl')
