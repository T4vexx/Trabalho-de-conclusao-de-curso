...
# Modelo Granite-Embedding-278M-Multilingual - IBM
GRANITE_MODEL = 'ibm-granite/granite-embedding-278m-multilingual'
BATCH_SIZE = 64  # 278M params, batch médio-grande
EMBEDDING_DIM = 768  # Dimensão fixa
MAX_SEQ_LENGTH = 512  # Limite padrão

# Diretórios
CHECKPOINT_DIR = 'checkpoints_granite'
RESULTS_DIR = 'results_granite'
EMBEDDINGS_DIR = 'embeddings_granite'
...
def get_granite_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings Granite-278M - IBM ENTERPRISE
    """
    ...
    # Encoding Granite (XLM-RoBERTa-like)
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings Granite (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {GRANITE_MODEL}", log_file)

            X = get_granite_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_granite.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_granite_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM GRANITE-EMBEDDING-278M-MULTILINGUAL", log_file)
    log_print(f"IBM Research | 278M params | Dim: 768 | 12 línguas", log_file)
    ...
    log_print(f"\n📥 Carregando Granite: {GRANITE_MODEL}", log_file)
    log_print(f"⚠️  Modelo 278M - XLM-RoBERTa-like (12 camadas)", log_file)
    log_print(f"⚠️  Apache 2.0 license - Enterprise friendly!", log_file)

    model = SentenceTransformer(GRANITE_MODEL, device=device, trust_remote_code=True)
    ...
    log_print(f"✓ Batch size: {BATCH_SIZE}", log_file)
    log_print(f"✓ Línguas: En, De, Es, Fr, Ja, Pt, Ar, Cs, It, Ko, Nl, Zh", log_file)
    ...
    test_texts = ["teste granite enterprise"] * 200
    start = time.time()
    ...
    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_granite.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_granite.pkl')
