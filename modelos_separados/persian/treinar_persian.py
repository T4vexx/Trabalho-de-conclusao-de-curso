...
# Modelo Persian-Embeddings - HEYDARI AI (PERSIAN + ENGLISH)
PERSIAN_MODEL = 'heydariAI/persian-embeddings'
BATCH_SIZE = 32  # 560M params (XLM-RoBERTa base), batch médio
EMBEDDING_DIM = 1024  # Dimensão alta!
MAX_SEQ_LENGTH = 512  # Padrão XLM-RoBERTa

# Diretórios
CHECKPOINT_DIR = 'checkpoints_persian'
RESULTS_DIR = 'results_persian'
EMBEDDINGS_DIR = 'embeddings_persian'
...
def get_persian_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings Persian-Embeddings - BILÍNGUE PERSIAN/ENGLISH
    """
    ...
    # Encoding XLM-RoBERTa fine-tuned para Persian
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings Persian (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {PERSIAN_MODEL}", log_file)

            X = get_persian_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_persian.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_persian_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM PERSIAN-EMBEDDINGS", log_file)
    log_print(f"HeydariAI | 560M params | Dim: 1024 | Persian + English", log_file)
    ...
    log_print(f"\n📥 Carregando Persian-Embeddings: {PERSIAN_MODEL}", log_file)
    log_print(f"⚠️  Fine-tuned XLM-RoBERTa para Persian/English", log_file)

    model = SentenceTransformer(PERSIAN_MODEL, device=device)
    ...
    log_print(f"✓ Batch size: {BATCH_SIZE}", log_file)
    log_print(f"✓ Base: XLM-RoBERTa (FacebookAI)", log_file)
    log_print(f"✓ Mean pooling com attention mask", log_file)
    ...
    test_texts = ["teste em português brasileiro"] * 200
    start = time.time()
    ...
    if velocidade < 20:
        ...
        log_print(f"   ✅ Velocidade OK!", log_file)
        tempo_estimado = 11902 / velocidade / 60
        log_print(f"   Tempo estimado: ~{tempo_estimado:.1f} minutos", log_file)

    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_persian.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_persian.pkl')
