...
RESULTS_DIR = 'results_bert'
EMBEDDINGS_DIR = 'embeddings_bert'
...
def get_bert_embeddings(texts, tokenizer, model, device, batch_size=16):
    """Gera embeddings BERT em batches usando GPU se disponível"""
    model.eval()
    embeddings = []

    with torch.no_grad():
        for i in tqdm(range(0, len(texts), batch_size), desc="Gerando embeddings BERT"):
            ...
            # Usar [CLS] token (primeira posição)
...
def process_dataset(dataset_path, tokenizer, model, device, checkpoint_manager, log_file):
    """Processa um dataset"""
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        # Verificar se embeddings já foram gerados
        if os.path.exists(embeddings_file):
            ...
            log_print(f"  Gerando embeddings BERT (usando {device})...", log_file)
            log_print(f"    Modelo: {BERT_MODEL}", log_file)
            log_print(f"    Batch size: {BATCH_SIZE}", log_file)

            X = get_bert_embeddings(texts, tokenizer, model, device, batch_size=BATCH_SIZE)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_bert.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_bert_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM BERT EMBEDDINGS", log_file)
    ...
    log_print(f"\nCarregando modelo BERT: {BERT_MODEL}", log_file)
    tokenizer = AutoTokenizer.from_pretrained(BERT_MODEL)
    model = AutoModel.from_pretrained(BERT_MODEL)
    model.to(device)
    log_print(f"Modelo carregado com sucesso!", log_file)

    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_bert.pkl')
    ...
    for dataset in DATASETS:
        if os.path.exists(dataset):
            results = process_dataset(dataset, tokenizer, model, device, checkpoint_manager, log_file)
            if results:
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_bert.pkl')
