...
# Modelo all-MiniLM-L6-v2 - SENTENCE-TRANSFORMERS (ULTRA RÁPIDO)
MINILM_MODEL = 'sentence-transformers/all-MiniLM-L6-v2'
BATCH_SIZE = 128  # Modelo SUPER leve (22M), batch GIGANTE!
EMBEDDING_DIM = 384  # Dimensão fixa (menor = mais rápido)
MAX_SEQ_LENGTH = 256  # Limite padrão (trunca)

# Diretórios
CHECKPOINT_DIR = 'checkpoints_minilm'
RESULTS_DIR = 'results_minilm'
EMBEDDINGS_DIR = 'embeddings_minilm'
...
def get_minilm_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings all-MiniLM-L6-v2 - ULTRA RÁPIDO
    """
    ...
    # Encoding MiniLM (mean pooling automático)
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings MiniLM (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {MINILM_MODEL}", log_file)

            X = get_minilm_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_minilm.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_minilm_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM ALL-MINILM-L6-V2", log_file)
    log_print(f"Sentence-Transformers | 22M params | Dim: 384 | ULTRA RÁPIDO", log_file)
    ...
    log_print(f"\n📥 Carregando MiniLM: {MINILM_MODEL}", log_file)
    log_print(f"⚠️  Modelo MAIS LEVE (22M) - RAPIDÍSSIMO!", log_file)
    ...
    log_print(f"✓ Batch size: {BATCH_SIZE} (GIGANTE!)", log_file)
    log_print(f"✓ Treinado em 1B+ pares de sentenças", log_file)
    ...
    allocated = torch.cuda.memory_allocated(0) / 1e9
    log_print(f"\n📊 VRAM alocada: {allocated:.2f} GB (MÍNIMA!)", log_file)

    # Teste de velocidade
    log_print(f"\n🧪 Teste de velocidade...", log_file)
    test_texts = ["teste ultra rápido"] * 200
    start = time.time()
    ...
    if velocidade < 50:
        ...
        log_print(f"   ✅ Velocidade EXTREMA - mais rápido que todos!", log_file)
        tempo_estimado = 11902 / velocidade / 60
        log_print(f"   Tempo estimado: ~{tempo_estimado:.1f} minutos", log_file)

    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_minilm.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_minilm.pkl')
