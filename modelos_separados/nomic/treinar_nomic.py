...
# Modelo Nomic-Embed-Text-v1.5 - NOMIC AI (LONG CONTEXT + MATRYOSHKA)
NOMIC_MODEL = 'nomic-ai/nomic-embed-text-v1.5'
BATCH_SIZE = 64  # 137M params, batch grande
EMBEDDING_DIM = 768  # Padrão (Matryoshka: 768→512→256→128→64)
MAX_SEQ_LENGTH = 2048  # CONTEXTO LONGO! (suporta 8192)
TASK_TYPE = 'search_document'  # ou 'search_query', 'clustering', 'classification'

# Diretórios
CHECKPOINT_DIR = 'checkpoints_nomic'
RESULTS_DIR = 'results_nomic'
EMBEDDINGS_DIR = 'embeddings_nomic'
...
def add_nomic_prefix(texts, task_type='search_document'):
    """
    Nomic requer prefixos de task:
    - search_document: para documentos
    - search_query: para queries
    - clustering: para agrupamento
    - classification: para classificação
    """
    return [f"{task_type}: {text}" for text in texts]

def get_nomic_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings Nomic-Embed-Text-v1.5 - LONG CONTEXT
    """
    ...
    log_print(f"    Task: {TASK_TYPE}", log_file)

    # Adicionar prefixo task-specific
    texts_with_prefix = add_nomic_prefix(texts, TASK_TYPE)
    log_print(f"    ✓ Prefixo Nomic adicionado ('{TASK_TYPE}:')", log_file)
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings Nomic (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {NOMIC_MODEL}", log_file)

            X = get_nomic_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_nomic.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_nomic_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM NOMIC-EMBED-TEXT-V1.5", log_file)
    log_print(f"Nomic AI | 137M params | Dim: 768 | Long Context 8192", log_file)
    ...
    log_print(f"\n📥 Carregando Nomic: {NOMIC_MODEL}", log_file)
    log_print(f"⚠️  Modelo 137M - long context BERT (8192 tokens!)", log_file)

    model = SentenceTransformer(NOMIC_MODEL, device=device, trust_remote_code=True)
    ...
    log_print(f"✓ Max seq length: {model.max_seq_length}", log_file)
    log_print(f"✓ Batch size: {BATCH_SIZE}", log_file)
    log_print(f"✓ Task type: {TASK_TYPE}", log_file)
    log_print(f"✓ Matryoshka: 768→512→256→128→64", log_file)
    ...
    test_texts = ["teste de contexto longo"] * 200
    start = time.time()
    _ = model.encode(
        add_nomic_prefix(test_texts, TASK_TYPE),
        batch_size=BATCH_SIZE,
    ...
    if velocidade < 30:
        ...
        log_print(f"   ✅ Velocidade EXCELENTE!", log_file)
        tempo_estimado = 11902 / velocidade / 60
        log_print(f"   Tempo estimado: ~{tempo_estimado:.1f} minutos", log_file)

    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_nomic.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_nomic.pkl')
