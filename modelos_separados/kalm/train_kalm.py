...
# Modelo KaLM-Embedding-V2.5 - LEVE E RÁPIDO
KALM_MODEL = 'KaLM-Embedding/KaLM-embedding-multilingual-mini-instruct-v2.5'
BATCH_SIZE = 32  # Modelo leve (0.5B), pode usar batch grande
EMBEDDING_DIM = 896  # Dimensão padrão do KaLM-v2.5
MAX_SEQ_LENGTH = 1024  # Contexto máximo de 32k, mas 512 é suficiente

# Diretórios
CHECKPOINT_DIR = 'checkpoints_kalm'
RESULTS_DIR = 'results_kalm'
EMBEDDINGS_DIR = 'embeddings_kalm'
...
def add_kalm_instruction(texts):
    """
    KaLM-v2.5 usa instruções task-specific
    Para classificação: "Instruct: Retrieve relevant documents for this text. Query: "
    """
    instruction = "Instruct: Retrieve relevant documents for this text. Query: "
    return [instruction + text for text in texts]

def get_kalm_embeddings(texts, model, batch_size, device, log_file=None):
    """
    Gera embeddings KaLM-v2.5 - OTIMIZADO PARA GPU
    """
    ...
    # Adicionar instruções KaLM
    texts_with_instruction = add_kalm_instruction(texts)
    log_print(f"    ✓ Instruções KaLM adicionadas", log_file)
...
def process_dataset(dataset_path, model, device, checkpoint_manager, log_file):
    ...
    for combo_name, fields in FIELD_COMBINATIONS.items():
        ...
        if X is None:
            ...
            log_print(f"  Gerando embeddings KaLM (USANDO GPU!)...", log_file)
            log_print(f"    Modelo: {KALM_MODEL}", log_file)

            X = get_kalm_embeddings(texts, model, BATCH_SIZE, device, log_file=log_file)
...
def save_results_summary(all_results, log_file):
    ...
    output_csv = os.path.join(RESULTS_DIR, 'resultados_kalm.csv')
...
def main():
    """Função principal"""
    log_file = os.path.join(RESULTS_DIR, f'log_kalm_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')

    log_print(f"{'='*80}", log_file)
    log_print(f"TREINAMENTO COM KALM-EMBEDDING-V2.5", log_file)
    log_print(f"Modelo: 0.5B params | Dim: 896 | Multilíngue", log_file)
    ...
    log_print(f"\n📥 Carregando KaLM-v2.5: {KALM_MODEL}", log_file)
    log_print(f"⚠️  Modelo leve (0.5B params), carregamento rápido!", log_file)

    model = SentenceTransformer(KALM_MODEL, device=device)
    ...
    checkpoint_file = os.path.join(CHECKPOINT_DIR, 'checkpoint_kalm.pkl')
    ...
    if all_results:
        ...
        results_pkl = os.path.join(RESULTS_DIR, 'resultados_completos_kalm.pkl')
